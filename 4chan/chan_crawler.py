        ...
        try:
            ...
            if 400 <= status_code < 500:
                if status_code == 404:
                    ...
                    logger.error(f"Client error (status {status_code}) occurred for thread {args[1]}. No retry.")
                break
    ...
    if thread_data is None:
        ...
        threads_collection.update_one(
            ...
            {"$set": {
                ...
                "number_of_replies": 0,
                "deleted_at": datetime.datetime.now(),
                "previous_title": existing_thread.get("original_post", "N/A"),  # Store the previous title/content before deletion
                "previous_replies": existing_thread.get("replies", [])  # Store the previous replies before deletion
            }},
            upsert=True
        )
